...
/**
 * PEMASARAN AKUN PAGE
 * Halaman utama untuk manajemen data Kunjungan Pemasaran (Marketing) khusus akun saat ini.
 */
export const PemasaranAkunPage: React.FC = () => {
  ...
  return (
    <MainShell 
      title="Riwayat Kunjunganku" 
      subtitle="Kelola laporan kunjungan pemasaran pribadi Anda"
      onAdd={() => navigate('/pemasaran-akun/tambah')}
      hideDownload={true}
      hideHeaderDivider={true}
      id="pemasaran-akun-main-page"
    >
      <div className="w-full space-y-4">
        <div className={cn("flex items-center gap-2", isMobile ? "flex-col items-stretch" : "")}>
          <div className={cn(isMobile ? "w-full" : "w-96")}>
            <SearchInput 
              id="pemasaran-akun-search-input"
              value={searchTerm}
              onSearch={(val) => setSearchTerm(val)}
              placeholder="Cari tipe, customer, atau alamat..."
        ...
        <Table id="pemasaran-akun-table" noBorder={true}>
          <TableHeader>
            ...
            {isLoading ? (
              ...
              data.map((row) => (
                <TableRow 
                  ...
                  onClick={() => navigate(`/pemasaran-akun/detail/${row.id}`)}
                >
                  ...
                  <TableCell noBorder={true} onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-center gap-1">
                      <GhostButton 
                        size="sm" 
                        onClick={() => navigate(`/pemasaran-akun/edit/${row.id}`)} 
                        title="Edit"
              ...
              <TableRow noBorder={true}>
                <TableCell colSpan={6} noBorder={true} className="h-48 text-gray-400 italic text-center">
                  Data kunjungan pemasaran pribadi tidak ditemukan
                </TableCell>
              </TableRow>
            )}
          </TableBody>
...
export default PemasaranAkunPage;
