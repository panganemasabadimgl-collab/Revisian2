...
import { IPiutang, IPiutangPembayaran, TPiutangStatus } from '../../../../logic/types/ITs_Piutang';
import { 
  AlertCircle, 
  CheckCircle2, 
  Plus
...
export const PiutangDetailPage: React.FC = () => {
  ...
  const [data, setData] = useState<(IPiutang & { payments: IPiutangPembayaran[] }) | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isViewModalOpen, setIsViewModalOpen] = useState(false);
  const [selectedPayment, setSelectedPayment] = useState<IPiutangPembayaran | null>(null);
  ...
  const handleViewPayment = (payment: IPiutangPembayaran) => {
  ...
  const handleAddPayment = async () => {
    ...
    if (paymentForm.amount > data.outstanding_amount) {
      swalToast.fire({ icon: 'error', title: 'Nominal cicilan tidak boleh melebihi sisa piutang' });
      return;
    }

    const confirm = await swalConfig.fire({
      title: 'Konfirmasi Pembayaran',
      text: 'Catatan pembayaran ini akan otomatis tercatat sebagai Pemasukan dan bersifat irreversible (tidak dapat dibatalkan). Lanjutkan?',
      icon: 'warning',
    ...
    const success = await piutangService.addPayment({
      piutang_id: id,
      ...paymentForm
    });

    if (success) {
      swalToast.fire({ icon: 'success', title: 'Berhasil! Pembayaran telah dicatat dan terhubung ke pemasukan.' });
  ...
  if (isLoading) {
    return (
      <DetailShell title="Memuat Detail Piutang..." onBack={() => navigate(-1)}>
    ...
    return (
      <DetailShell title="Data Tidak Ditemukan" onBack={() => navigate(-1)}>
        <div className="flex flex-col items-center justify-center h-64 text-TextColorMuted">
          ...
          <p>Maaf, data piutang tidak tersedia.</p>
        </div>
      </DetailShell>
    );
  }

  const isPaid = data.status === TPiutangStatus.SETTLED;

  return (
    <DetailShell 
      title={data.name} 
      onBack={() => navigate(referrer || '/finansial/piutang')}
      id="piutang-detail-shell"
    >
      <div className="p-SpacingBase space-y-SpacingBase">
        ...
        <div className="bg-ColorPrimary/10 rounded-RadiusLarge p-SpacingMedium border border-ColorPrimary/5">
            ...
            <div className="grid grid-cols-3 gap-SpacingMedium">
                ...
                <div>
                  <Label id="lbl-sisa-piutang" className="text-red-500">Sisa Piutang</Label>
        ...
        <div className="bg-ColorBg rounded-RadiusLarge p-SpacingMedium border border-ColorPrimary/5">
            <div className="flex justify-between items-center mb-SpacingMedium">
                <h3 className="text-FontSizeSm font-bold">Log Pembayaran Piutang</h3>
            ...
            <div className="space-y-SpacingSmall">
              ...
              {data && paymentForm.amount > data.outstanding_amount && (
                <div className="text-red-500 text-xs font-medium">
                  Nominal melebihi sisa piutang ({formatCurrency(data.outstanding_amount)})
                </div>
              )}
            </div>
            <div className="space-y-SpacingSmall">
...
export default PiutangDetailPage;
